...
function verificarPassword($idUsuario, $password){
    ...
    $contrasena = select($sentencia, [$idUsuario])[0]->password;
    $verifica = password_verify($password, $contrasena);
...
function registrarUsuario($usuario, $nombre, $telefono, $direccion){
    $sentencia = "INSERT INTO usuarios (usuario, nombre, telefono, direccion) VALUES (?,?,?,?)";
    $parametros = [$usuario, $nombre, $telefono, $direccion];
